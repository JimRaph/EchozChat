...
const Profile = () => {
  ...
  const handleClickOutside = (event) => {
    ...
    const isOutsideEmojiElements =
      ...
      (!emojiTriggerDiv.current ||
        !emojiTriggerDiv.current.contains(event.target));
  ...
  const handleClickOutside1 = (event) => {
    ...
    const isOutsideEmojiElements =
      (!emojiTriggerRef1.current ||
        !emojiTriggerRef1.current.contains(event.target)) &&
      (!emojiPickerRef1.current ||
        !emojiPickerRef1.current.contains(event.target)) &&
      (!emojiTriggerDiv.current ||
        !emojiTriggerDiv.current.contains(event.target));
  ...
  const updateProfile = async ({
    ...
    try {
      ...
      const { data } = await axios.put(`${base_url}/users/profile`, formData, {
        headers: {
  ...
  return (
    <div>
      ...
      <div className="relative p-1 mb-4">
        ...
        {showEmojiPicker && (
          <div ref={emojiPickerRef} className=" fixed left-10 top-130 z-50">
            <EmojiPicker
      ...
      <div className="relative p-1">
        ...
        {showEmojiPicker1 && (
          <div ref={emojiPickerRef1} className=" fixed left-10 top-130 z-50">
            <EmojiPicker
      ...
      <div className="flex flex-col mt-6 space-y-2">
        <button
          onClick={logout}
          className={`${theme.type === "Dark" ? "text-red-400" : theme.textPrimary} rounded-md ${theme.secondary} w-fit
               p-1 pl-6 pr-6 border ${theme.border}`}
        >
          Log out
        </button>
        <p className={`${theme.textPrimary} text-xs`}>
          Chat history on this computer will be cleared when you
          <br /> log out.
        </p>
      </div>
