...
const Contextmenu = ({ modalPosition, setShowModal, selectedContextChat }) => {
  ...
  const getStatus = (field) =>
    Array.isArray(selectedContextChat?.[field])
      ? selectedContextChat[field].find((c) => c.user === user._id)?.status ==
        true
      : false;
  ...
  const markAsRead = async () => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `${base_url}/chats/${selectedContextChat._id}/read`,
        {},
        {
          headers: {
  ...
  const pinToTop = async () => {
    ...
    updateLocalChat(selectedContextChat._id, {
      pinned: [{ user: user._id, status: newStatus }],
    });
    ...
    try {
      ...
      const { data } = await axios.put(
        `${base_url}/chats/${selectedContextChat._id}/pin`,
        {},
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("EchozChat-token")}`,
  ...
  const addToFavorite = async () => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `${base_url}/chats/${selectedContextChat._id}/favourite`,
        {},
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("EchozChat-token")}`,
  ...
  const archiveChat = async () => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `${base_url}/chats/${selectedContextChat._id}/archive`,
        {},
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("EchozChat-token")}`,
  ...
  const clearMessages = async () => {
    ...
    try {
      ...
      const { data } = await axios.delete(
        `${base_url}/chats/${selectedContextChat._id}/messages`,
    ...
    >
      <ul className="text-white text-sm">
        <li
          ...
          className={`flex items-center space-x-2 p-2 hover:bg-[#30393f] cursor-pointer border-b border-gray-600 ${
        ...
        <li
          ...
          className={`flex items-center space-x-2 p-2 hover:bg-[#30393f] cursor-pointer ${
            isLoading ? "opacity-50" : ""
          }`}
        >
          {!isPinned ? (
