...
const ContextProvider = ({ children }) => {
  ...
  const [token, setToken] = useState(() => {
    const storedToken = localStorage.getItem("EchozChat-token");
    if (storedToken) {
      try {
  ...
  const [user, setUser] = useState(() => {
    return JSON.parse(localStorage.getItem("EchozChat-user"));
  });
  ...
  const createChat = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        localStorage.setItem("EchozChat-chat", JSON.stringify(data.chat));
      } else {
        toast.error(data?.message || "Something went wrong");
      }
    } catch (error) {
  ...
  const identifier = (chat) => {
    ...
    const saved_identifier = contacts?.filter(
      (c) => c.contact?._id == identifier?._id
    );
    let identifier_name;
    ...
    const identity = identifier_name ? identifier_name : identifier?.Phone;
  ...
  const getUnreadCount = (chat) => {
    const myCount = chat.unreadCounts?.find(
      (uc) => uc.user.toString() === user._id.toString()
    );
  ...
  const scrollToMessage = (message, messageId) => {
    ...
    setTimeout(() => {
      ...
      if (el) {
        ...
        setTimeout(
          () => el.classList.remove("rounded-md", "bg-blue-50/10"),
          800
        );
      }
    }, 1000);
