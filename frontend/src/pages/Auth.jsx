...
const AuthFlow = ({onAuthenticated}) => {
  ...
  const handlePhoneSubmit = async(e) => {
    ...
    try{
      const payloadPhone = isTestNumber ? phoneNumber : selectedCountry.dial_code + phoneNumber;

      const response = await axios.post(base_url+'/auth/register', {Phone: payloadPhone} )
  ...
  const handleOtpSubmit = async(e) => {
    ...
    try {
      ...
      const {data} = await axios.post(base_url+'/auth/verify', 
        {phone: payloadPhone, otp: otp.join('')})
            ...
            <form onSubmit={handlePhoneSubmit} className="space-y-4">
              ...
              <div className="relative">
                <button
                  ...
                  className={`w-full flex items-center justify-between p-3 rounded ${theme === 'Dark' ? 'bg-[#394f5e] text-gray-50' : 'bg-[#F3F3F3] text-black'} outline-none`}
                >
                  ...
                  <ChevronDown size={20} className={`${theme.textSecondary} flex-shrink-0 transition-transform ${showCountryDropdown ? 'rotate-180' : ''}`} />
                </button>
                
                {showCountryDropdown && (
                  <div className={`absolute left-0 top-full mt-1 w-full max-h-60 overflow-y-auto rounded shadow-lg z-50 ${theme === 'Dark' ? 'bg-[#2a3942] text-gray-50' : 'bg-white text-black'}`}>
                    {countries.map((country) => (
                      <div
                        ...
                        className={`p-3 cursor-pointer flex items-center justify-between ${theme === 'Dark' ? 'hover:bg-[#202c33]' : 'hover:bg-gray-100'}`}
                      >
                         <span>{country.name} ({country.dial_code})</span>
                         {selectedCountry.code === country.code && <Check size={16} className="text-[#00a884]" />}
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className={`flex items-center p-3 ${theme === 'Dark' ? 'bg-[#394f5e] text-gray-50' : 'bg-[#F3F3F3]'} rounded`}>
                <span className={`mr-2 ${theme === 'Dark' ? 'text-gray-400' : 'text-gray-600'}`}>{selectedCountry.dial_code}</span>
                <input
                  type="tel"
                  inputMode="numeric"
                  pattern="[0-9]*"
                  ...
                  className={`w-full bg-transparent outline-none ${theme === 'Dark' ? 'text-gray-50' : 'text-black'}`}
                />
              </div>
          ...
          <div className="w-full">
            <div className="text-center mb-8">
              ...
              <p className={` ${theme.textSecondary} ${theme=== 'Dark' ? 'text-gray-400' :''} `}>
                We sent a code to {['1234', '2233', '5678'].includes(phoneNumber) ? phoneNumber : selectedCountry.dial_code + phoneNumber}
              </p>
              {verificationCode && (
